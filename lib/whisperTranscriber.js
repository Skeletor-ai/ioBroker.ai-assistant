...
class WhisperTranscriber {
    ...
    async init() {
        ...
        try {
            ...
            // Download model if not present (smart-whisper stores in ~/.smart-whisper/models/)
            if (!manager.check(this.model)) {
                this.log.info(`Downloading whisper model "${this.model}"...`);
                await manager.download(this.model);
                this.log.info(`Model "${this.model}" downloaded to ${manager.dir.models}`);
            }

            // Resolve model path from smart-whisper's model store
            const modelPath = manager.resolve(this.model);
            this.log.debug(`Using model file: ${modelPath}`);
    ...
    async transcribe(audioBuffer, format = 'ogg') {
        ...
        // Transcribe
        // Use available CPU threads (default to 4, capped at CPU count)
        const cpuCount = require('os').cpus().length;
        const nThreads = Math.min(Math.max(cpuCount, 1), 8);

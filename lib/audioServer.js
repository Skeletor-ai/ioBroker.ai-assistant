...
class AudioServer {
    ...
    /**
     * Publish mDNS service for auto-discovery by clients (e.g. Kalima app).
     * @private
     */
    _publishMdns() {
        try {
            ...
            this._mdns.publish({
                name: `ioBroker AI Assistant (${os.hostname()})`,
                type: 'iobroker-ai',
                port: this.port,
                txt: {
                    path: '/audio',
            ...
            this.log.info(`mDNS service published: _iobroker-ai._tcp on port ${this.port}`);
        } catch (e) {
            this.log.warn(`mDNS not available (install bonjour-service for auto-discovery): ${e.message}`);
        }
    }

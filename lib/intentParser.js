...
class IntentParser {
    ...
    async parse(text) {
        ...
        // Skip filter when "alle/alles/sämtliche/every/all" is in the text → target ALL states
        let deviceName = null;
        const allPattern = /\b(alle[snm]?|sämtliche[snm]?|every|all)\b/;
        if (stateIds.length > 1 && !allPattern.test(lower)) {
     ...
     */
    _detectAction(lower) {
        // IMPORTANT: Check for QUESTIONS first!
        // "Ist das Licht an?" must NOT be parsed as set_on.
        // Questions are read-only and should go to the LLM for proper handling.
        ...
        // QUERY already handled at the top of _detectAction

...
class TemplateEngine {
    ...
    matchTemplate(userText) {
        ...
        if (bestMatch) {
            this.log.info(`Matched template "${bestMatch.id}" (score: ${bestScore})`);
        } else {
            // Log all templates and their trigger words for debugging
            for (const template of this._templates.values()) {
                this.log.debug(`Template "${template.id}": triggerWords=[${(template.triggerWords || []).join(', ')}]`);
            }
            this.log.debug(`No template matched for "${lower}"`);
        }
    ...
    async buildContext(template) {
        ...
        for (const source of template.contextSources) {
            ...
            try {
                ...
                this.log.debug(`Context source "${source.label || source.pattern}": ${stateIds.length} states`);
    ...
    async _resolvePattern(pattern) {
        ...
        try {
            // First try with type filter 'state'
            let objects = await this.adapter.getForeignObjectsAsync(prefix + '*', 'state');
            let ids = Object.keys(objects).filter((id) => this._matchPattern(id, pattern));

            // system.* states may not have type='state' in all ioBroker versions
            // Fall back to unfiltered query if no results
            if (ids.length === 0) {
                this.log.debug(`Pattern "${pattern}": no type=state matches for prefix "${prefix}*", trying unfiltered...`);
                objects = await this.adapter.getForeignObjectsAsync(prefix + '*');
                const allIds = Object.keys(objects);
                this.log.debug(`Pattern "${pattern}": unfiltered returned ${allIds.length} objects`);
                ids = allIds.filter((id) => this._matchPattern(id, pattern));
            }

            this.log.debug(`Pattern "${pattern}": resolved ${ids.length} state(s)`);
            return ids.sort();
        } catch (e) {
            this.log.warn(`Pattern resolve ${pattern}: ${e.message}`);
            return [];
        }
    }
